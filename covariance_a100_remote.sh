...
run_and_pull() {
  ...
  ssh_cmd "set -e; cd '${REMOTE_REPO}'; mkdir -p data/second_moment_stats; source \$HOME/miniconda3/etc/profile.d/conda.sh; PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True conda run -n '${CONDA_ENV}' python -m src.cli command=second-moment model='${model_key}' model.device=cuda ++model.cuda_mode=strict 2>&1 | tee '${REMOTE_REPO}/$log_name'"
