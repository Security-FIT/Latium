...
usage() {
  sed -n '1,180p' "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/README.md"
}
...
case "$PRESET" in
  ...
  analyze)
    ...
    SCRATCH=20gb
    WALLTIME=12:00:00
    DEFAULT_ARGS=(structural analyze)
    ;;
  custom)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --) shift; break ;;
    -h|--help) usage; exit 0 ;;
    *) die "unknown submission option '$1'; put command or pipeline arguments after --" ;;
  esac
done
