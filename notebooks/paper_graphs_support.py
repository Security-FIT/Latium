...
def find_repo_root(start: Optional[Path] = None) -> Path:
    ...
    if (root / "notebooks" / "paper_graphs_support.py").exists():
        return root
    if (root / "notebooks" / "new-gen" / "_newgen_utils.py").exists():
        return root
...
def plot_model_stack(
    ...
    if save_figures:
        if output_dir is None:
            output_dir = find_repo_root() / "notebooks" / "paper_graphs_output"
        output_dir.mkdir(parents=True, exist_ok=True)
        out_path = output_dir / f"{model}_paper_graphs.png"
