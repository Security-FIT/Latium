#!/usr/bin/env python3
"""Generate matched non-ROME edits for the one-checkpoint Gram experiment."""
...
from src.structural.experiments.simple_gram import (
    SCHEMA_VERSION as SIMPLE_GRAM_SCHEMA,
)
from src.structural.experiments.simple_gram import capture_simple_gram


SCHEMA_VERSION = "rome-simple-gram-hard-negatives-v1"
CATEGORIES = (
...
def _magnitude_targets(path: Path) -> list[dict[str, Any]]:
    """Read ROME Gram magnitudes from a historical capture or prior bundle."""
    payload = json.loads(path.read_text(encoding="utf-8"))
    targets: list[dict[str, Any]] = []
    seen: set[str] = set()
    if payload.get("schema_version") == "rome-single-checkpoint-hard-negatives-v1":
        for record in payload.get("records", ()):
            case_id = record.get("source_rome_case_id")
            update = record.get("update") or {}
            magnitude = update.get("target_change_magnitude")
            if magnitude is None:
                magnitude = (update.get("first") or {}).get(
                    "target_change_magnitude"
                )
            if case_id is None or magnitude is None or str(case_id) in seen:
                continue
...
def _gram_delta_terms(
    ...
    left_device = left.to(device=matrix.device)
    right_device = right.to(device=matrix.device)
    base = hidden_gram(matrix, normalize=False)
    if matrix.shape[0] <= matrix.shape[1]:
        projected = matrix @ right_device
        linear = (
            projected @ left_device.T + left_device @ projected.T
        )
        quadratic = (
            left_device
            @ (right_device.T @ right_device)
            @ left_device.T
        )
    else:
        projected = matrix.T @ left_device
        linear = (
            projected @ right_device.T + right_device @ projected.T
        )
        quadratic = (
            right_device
            @ (left_device.T @ left_device)
            @ right_device.T
        )
    base_norm = float(torch.linalg.matrix_norm(base, ord="fro").item())
    return linear, quadratic, base_norm
...
def _matched_update(
    ...
    *,
    ...
    """Match a random low-rank update to a ROME hidden-Gram magnitude."""
    ...
    actual_delta = hidden_gram(
        edited,
        normalize=False,
    ) - hidden_gram(weight, normalize=False)
    actual = (
        float(torch.linalg.matrix_norm(actual_delta, ord="fro").item())
        / base_norm
    )
    return edited, {
        ...
        "relative_match_error": (
            abs(actual - float(target_magnitude)) / float(target_magnitude)
        ),
        "update_frobenius": float(
            torch.linalg.matrix_norm(update.float(), ord="fro").item()
        ),
    }


def _capture(weights: dict[int, torch.Tensor]) -> dict[str, Any]:
    capture = capture_simple_gram(
        CaptureContext(
    ...
    if capture["schema_version"] != SIMPLE_GRAM_SCHEMA:
        raise RuntimeError("Unexpected simple-Gram capture schema")
    return capture
...
def generate(
    *,
    model_key: str,
    magnitude_source: Path,
    count: int,
    seed: int,
) -> dict[str, Any]:
    targets = _magnitude_targets(magnitude_source)
    cfg = build_cfg(
        model_key,
    ...
    configured_fc = str(
        getattr(cfg.model, "fc_layer_name_template", "") or ""
    )
    fc_template = configured_fc or get_fc_template(projection_template)
    fc_weight = (
        handler._get_module(
            fc_template.format(int(cfg.model.layer))
        ).weight.detach().clone().cpu()
        if fc_template
        else None
    )
    ...
    other_layers = [layer for layer in sorted(projection) if layer != target_layer]
    second_layer = min(
        other_layers,
    ...
    for category_index, category in enumerate(CATEGORIES):
        for index in range(count):
            ...
            generator = torch.Generator().manual_seed(
                seed + 100_000 * category_index + index
            )
            ...
            if category == "matched_random_rank_one":
                ...
                if fc_weight is None:
                    raise RuntimeError(
                        f"{model_key} has no distinct non-target FC matrix"
                    )
    ...
    return {
        ...
        "magnitude_source": {
            "path": str(magnitude_source),
            "sha256": _sha256(magnitude_source),
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Historical ROME magnitude capture or prior hard-negative bundle; "
            "used only to size synthetic edits."
        ),
    )
