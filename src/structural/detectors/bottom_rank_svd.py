...
class BottomRankSVDDetector:
    ...
    def detect(
        ...
        scored = score_token_sweeps(token_id_sweeps, boundary=self.boundary)
        if not scored["layer_scores"]:
            out = self._empty()
            out["skipped_layers"] = skipped_layers
            return out

        return {
            "anomalous_layer": int(scored["anomalous_layer"]),
            "detection_score": float(scored["detection_score"]),
            "probe_dim": int(probe.numel()),
            "unique_prediction_counts": scored["unique_prediction_counts"],
            "switch_counts": scored["switch_counts"],
            "switch_rates": scored["switch_rates"],
