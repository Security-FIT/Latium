"""
Matrix-anomaly cross-layer detection metrics for ROME edit detection.
...
Uses cross-layer matrix-profile anomaly scoring to detect the edited layer.
All metrics are blind (no baseline needed).
...
def compute_matrix_anomaly_metrics(
    weights: Dict[int, torch.Tensor],
    fc_weights: Optional[Dict[int, torch.Tensor]] = None,
    boundary: int = 2,
) -> Dict:
    """
    Compute matrix-anomaly detection metrics across all layers.
    ...
    return {
        ...
        "per_metric_series": {
            name: {str(layer): value for layer, value in series.items()} for name, series in per_metric_series.items()
        },
        "per_metric_z_scores": {
            name: {str(layer): value for layer, value in series.items()} for name, series in per_metric_z.items()
        },
        "per_metric_curvature": {
            name: {str(layer): value for layer, value in series.items()} for name, series in per_metric_curv.items()
        },
        "per_metric_mad_scores": {
            name: {str(layer): value for layer, value in series.items()} for name, series in per_metric_mad.items()
        },
        "contrast_series": {
            name: {str(layer): value for layer, value in series.items()} for name, series in contrast_series.items()
        }
        if contrast_series
        else {},
