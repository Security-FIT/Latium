...
def matrix_basic_profile(weight: torch.Tensor) -> dict[str, Any]:
    """Non-SVD per-layer matrix profile used by captures and detectors."""
...
def matrix_svd_profile(
    weight: torch.Tensor,
    singular_values: Optional[np.ndarray] = None,
    frob_sq: Optional[float] = None,
) -> dict[str, Any]:
    """SVD-derived per-layer matrix profile used by captures and detectors."""
    ...
    if frob_sq is None:
        matrix = weight.detach().float() if matrix is None else matrix
        frob_sq = float(matrix.square().sum().item())
...
def matrix_profile(
    ...
    basic = matrix_basic_profile(weight)
    return {
        **basic,
        **matrix_svd_profile(
            weight,
            singular_values=singular_values,
            frob_sq=float(basic["frob_norm"]) ** 2,
        ),
    }
