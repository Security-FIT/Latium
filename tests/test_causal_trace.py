...
def test_subject_noise_calibration_uses_actual_subject_tokens_with_multiplicity() -> None:
    tokenizer = _ToyTokenizer()
    examples = [
        TraceExample("0", "Ada wrote code", "Ada", "math"),
        TraceExample("1", "Bob wrote prose", "Bob", "text"),
        TraceExample("2", "Ada made notes", "Ada", "notes"),
        TraceExample("3", "Ada met Ada", "Ada", "meeting"),
    ]
    ada_id = tokenizer("Ada")["input_ids"][0]
    bob_id = tokenizer("Bob")["input_ids"][0]
    embedding = torch.nn.Embedding(1001, 2)
    with torch.no_grad():
        embedding.weight.zero_()
        embedding.weight[ada_id] = torch.tensor([1.0, 3.0])
        embedding.weight[bob_id] = torch.tensor([5.0, 7.0])
    handler = type("Handler", (), {"tokenizer": tokenizer})()

    calibration = calibrate_subject_noise(handler, embedding, examples, multiplier=3.0)

    expected = torch.tensor([1.0, 3.0, 5.0, 7.0, 1.0, 3.0], dtype=torch.float64).std().item()
    assert calibration.embedding_std == pytest.approx(expected)
    assert calibration.noise_std == pytest.approx(3.0 * expected)
    assert calibration.num_subjects == 3
    assert calibration.num_subject_tokens == 3
    assert calibration.num_unique_token_ids == 2
    assert calibration.num_rejected_subjects == 1
    assert calibration.source == "candidate_subject_token_embeddings"
...
class _ToyProjection(torch.nn.Module):
...
class _ToyCausalModel(torch.nn.Module):
    def __init__(self, num_layers: int = 4) -> None:
        ...
        self.layers = torch.nn.ModuleList([_ToyProjection() for _ in range(num_layers)])
        with torch.no_grad():
            self.embed.weight.zero_()
            self.embed.weight[1] = torch.tensor([1.0, -1.0])

    def forward(self, input_ids, **_kwargs):
        hidden = self.embed(input_ids)
        for layer in self.layers:
            hidden = layer(hidden)
        distance = (hidden[:, 0, :] - torch.tensor([1.0, -1.0], device=hidden.device)).square().sum(dim=1)
        logits = torch.zeros(input_ids.shape[0], input_ids.shape[1], 16, device=hidden.device)
        logits[:, -1, 3] = 5.0 - 2.0 * distance
        return SimpleNamespace(logits=logits)


class _ToyTraceTokenizer:
    bos_token_id = None
    _ids = {"Ada": 1, "is": 2, "answer": 3}

    def __call__(self, text, add_special_tokens=True, return_offsets_mapping=False, return_tensors=None):
        del add_special_tokens
        words = text.strip().split()
        ids = [self._ids.get(word, 4) for word in words]
        payload = {"input_ids": ids}
        if return_offsets_mapping:
            offsets = []
            cursor = 0
    ...
    def decode(self, ids, skip_special_tokens=False):
        ...
        return " ".join(reverse.get(int(token_id), "other") for token_id in ids)


class _ToyTraceHandler:
    def __init__(self) -> None:
        self.model = _ToyCausalModel()
        self.tokenizer = _ToyTraceTokenizer()
...
def test_model_free_pipeline_writes_auditable_paired_trace(tmp_path: Path, monkeypatch) -> None:
    import importlib

    trace_module = importlib.import_module("src.causal_trace.causal_trace")
    examples = [TraceExample(str(index), "Ada is", "Ada", "answer") for index in range(4)]
    monkeypatch.setattr(trace_module, "_dataset_examples", lambda _cfg, max_scan=None: iter(examples[:max_scan]))
    cfg = OmegaConf.create(
        {
            "model": {
                "name": "toy/model",
                "layer": 2,
                "corrupt_layer_name_template": "embed",
            },
            "command": {
                "causal_trace": {
                    "output_dir": str(tmp_path),
                    "num_valid_facts": 4,
                    "max_dataset_examples_to_scan": 4,
                    "num_noise_samples": 4,
                    "noise_batch_size": 2,
    ...
    out_dir = trace_module._run_causal_trace(cfg, _ToyTraceHandler())
    ...
    facts = [json.loads(line) for line in (out_dir / "fact_results.jsonl").read_text().splitlines()]

    assert summary["configured_reference_layer_used_for_selection"] is False
    assert summary["noise_scale_source"] == "candidate_subject_token_embeddings"
    assert summary["noise_calibration_num_subjects"] == 4
    assert summary["num_discovery_facts"] == summary["num_confirmation_facts"] == 2
    assert selection["selected_trace_center"] == selection["discovery_trace_center"] == 1
    assert selection["confirmation_passed"] is True
    assert len(facts) == 4
    assert np.asarray(facts[0]["window_restore_probabilities"]).shape == (4, 4)
    assert len(facts[0]["corrupt_probabilities"]) == 4
    assert facts[0]["total_effect"] > 0
